...
from datasets.crowdhuman.register_crowdhuman import *
...
class Trainer(DefaultTrainer):
    ...
    def filter_person_detections(results):
        """
        过滤检测结果，仅保留COCO类别ID=1（人）的预测框（GPU版本）
        Args:
            results: list[dict]，Detectron2标准检测结果，每个元素包含"instances"字段
        Returns:
            filtered_results: 过滤后的结果（仍在GPU上）
        """
        filtered_results = []
        for res in results:
            ...
            # 直接使用GPU上的instances，不转CPU
            instances = res["instances"]
            # 掩码计算在GPU上执行（pred_classes本身就在GPU上）
            person_mask = instances.pred_classes == 1
            filtered_instances = instances[person_mask]
            
            # 替换原结果中的instances（仍保留在GPU）
            res["instances"] = filtered_instances
            filtered_results.append(res)
    ...
    def test(cls, cfg, model, evaluators=None):
        ...
        if evaluators is None:
            assert len(cfg.DATASETS.TEST) == 1, "Only support single dataset evaluation!"
            dataset_name = cfg.DATASETS.TEST[0]
            output_folder = os.path.join(cfg.OUTPUT_DIR, "inference")
            evaluator = cls.build_evaluator(cfg, dataset_name, output_folder)
            evaluators = [evaluator]
        
        # 调用父类推理方法
        results = super().test(cfg, model, evaluators)
        
        # 核心修改：过滤非行人类别（仅保留category_id=0）
        for dataset_name in results.keys():
            if isinstance(results[dataset_name], list):
                filtered_predictions = []
                for pred in results[dataset_name]:
                    # 只保留类别ID=0的检测框
                    pred["instances"] = pred["instances"][pred["instances"].pred_classes == 0]
                    filtered_predictions.append(pred)
                results[dataset_name] = filtered_predictions
